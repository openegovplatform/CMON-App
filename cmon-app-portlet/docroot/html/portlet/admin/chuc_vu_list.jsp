...
<%
	 ...
	 searchUrl.setParameter(ActionRequest.ACTION_NAME, "searchChucVuRequest");
	 
	 PortletURL addNewUrl = renderResponse.createRenderURL();
	 addNewUrl.setParameter("ActionType", "addChucVu");
	 addNewUrl.setParameter("jspPage", "/html/portlet/admin/chuc_vu_add.jsp");
	 
	 String keyWord = renderRequest.getParameter("searchKeyWord");
	 
	 // Get default search value
	 String defaultSearch = LanguageUtil.get(pageContext, "vn.dtt.cmon.admin.dmcv.search.defaultValue");
	 if (keyWord == null) {
	 ...
	 iteratorUrl.setParameter("jspPage", "/html/portlet/admin/chuc_vu_list.jsp");
	 ...
	 if (request.getAttribute("SEARCH_REQUEST_RESULT") != null) {
		 result = (List<ChucVu>)request.getAttribute("SEARCH_REQUEST_RESULT");
	 } else {
		 if (renderRequest.getParameter("searchKeyWord") != null) {
			 result = ChucVuBusiness.findByTenChucVu(renderRequest.getPortletSession().getAttribute("searchKeyWord").toString());
		 }
	 }
	...
	<div class="wd-main-content" align="center">
		...
		<form name = "searchForm" method = "post" action = "<%=searchUrl%>" onsubmit = "checkForm2()">
			<table width="100%" border="0">
			  ...
			  <tr>
			    <td>
			    	<liferay-ui:error key="errorDeleteVaiTro" message = "vn.dtt.cmon.admin.dmcv.error.delete"/>
			    	<%
				if (request.getAttribute("delete_success") != null && request.getAttribute("delete_success").toString().trim().length() > 0) {
					%>
					<font color="blue">
						<liferay-ui:message key="vn.dtt.cmon.admin.dmcv.completed.delete"/>
					</font>					
					<%
				}
				%>
	...
	<%
		for(ChucVu quocGia : newResults) {											
			
			PortletURL viewRequestUrl = renderResponse.createRenderURL();
			viewRequestUrl.setParameter("ActionType", ChucVuBusiness.VIEW_CHUC_VU);
			viewRequestUrl.setParameter("jspPage", "/html/portlet/admin/chuc_vu_add.jsp");
			viewRequestUrl.setParameter("ChucVuId", String.valueOf(quocGia.getId()));
			if (renderRequest.getParameter("searchKeyWord") != null) {
			...
			editRequestUrl.setParameter("ActionType", "editChucVu");		
			editRequestUrl.setParameter("jspPage", "/html/portlet/admin/chuc_vu_add.jsp");
			editRequestUrl.setParameter("ChucVuId", String.valueOf(quocGia.getId()));
			if (renderRequest.getParameter("searchKeyWord") != null) {
				editRequestUrl.setParameter("searchKeyWord", renderRequest.getParameter("searchKeyWord"));
			}
			
			PortletURL deleteUrl = renderResponse.createActionURL();
			deleteUrl.setParameter(ActionRequest.ACTION_NAME, "deleteChucVuRequest");
			deleteUrl.setParameter("jspPage", "/html/portlet/admin/dmvt_list.jsp");
			deleteUrl.setParameter("ChucVuId", String.valueOf(quocGia.getId()));
			if (renderRequest.getParameter("searchKeyWord") != null) {
