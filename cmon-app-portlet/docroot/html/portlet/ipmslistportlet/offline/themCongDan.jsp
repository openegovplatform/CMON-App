		...
		<table  class="egov-table-form">
			...
			<tr>
				<td colspan="2" align = "center">
					...
					<liferay-ui:error key="invalidNgaySinh" message = "vn.dtt.form.offline.invalid.ngaysinh"/>
			...
			<tr>
				<td>
					<label class ="egov-label-bold">Địa chỉ liên hệ <font color="red">(*)</font>:</label>
				</td>
				<td>
			...
			<tr>
                ...
                <td>
                    <input id="phoneNumber" name="phoneNumber" class = "egov-inputfield custom" value="<%=ConvertUtil.getValueFromRenderRequest("phoneNumber", renderRequest) %>" type="text" maxlength="11" size="50"/>
                </td>
            </tr>
			<tr>
				<td>
				...
				<td>
					<select name="idTinhHienTai" id="idTinhHienTai"  onchange="reDrawSelectBox('idTinhHienTai','idHuyenHienTai')" class = "egov-select" style="width:227px; display:block;">
						...
						String idTinhMacDinh = "31";
						if (renderRequest.getParameter("idTinhHienTai") != null) {
							idTinhMacDinh = renderRequest.getParameter("idTinhHienTai");
						}
						
						for(DonViHanhChinh donViHanhChinh : listTinh) {
							%>
							<option <%=ConvertUtil.checkSelectedValue(idTinhMacDinh, donViHanhChinh.getId()) %> value = "<%=donViHanhChinh.getId()%>"><%=donViHanhChinh.getTen()%></option>
							<%
						}
						%>
					</select>
			...
			<tr>
				...
				<td>
					<select name="idHuyenHienTai" id="idHuyenHienTai"  onchange="reDrawSelectBox('idHuyenHienTai','idXaHienTai')" class = "egov-select" style="width:227px; display:block;">
						...
						<%
						if (idTinhMacDinh != null) {
							List<DonViHanhChinh> dsDVHC = formOfflineBusiness.findDonViHanhChinhByChaId(ConvertUtil.convertToLong(idTinhMacDinh));
