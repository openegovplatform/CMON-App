...
actionUrl.setParameter(ActionRequest.ACTION_NAME, "searchCongDanNop");

String searchKeyWord = "";
if (renderRequest.getParameter("searchKeyWord") != null) {
	searchKeyWord = renderRequest.getParameter("searchKeyWord");
}
String searchKeyWordOld = "";
if (renderRequest.getParameter("searchKeyWordOld") != null) {
	searchKeyWordOld = renderRequest.getParameter("searchKeyWordOld");
}
...
addCongDanRequestUrl.setParameter("jspPage", "/html/portlet/ipmslistportlet/company/themCongDan.jsp");
...
addCongDanRequestUrl.setParameter("idCongDan",idCongDan);
addCongDanRequestUrl.setParameter("idCongDanNop",idCongDanNop);
...
<div class="egov-container">
		...
		<table style="width: 100%; background-color: white;">
			<tr>
				<td align="left" width="100%">
					<table  style="width: 100%; background-color: white;">
						<tr>
							<td align="left"><label class="egov-label-header">Tìm thông tin công dân</label></td>
							<td align="right">
								<%
									PortletURL nopHoSoOfflineUrl = renderResponse.createRenderURL();
									nopHoSoOfflineUrl.setParameter("jspPage", "/html/portlet/ipmslistportlet/company/inputHoSo.jsp");
									nopHoSoOfflineUrl.setParameter("searchKeyWord", searchKeyWordOld);
									nopHoSoOfflineUrl.setParameter("_searchKeyWord", _searchKeyWord);
									nopHoSoOfflineUrl.setParameter("idCongDan",idCongDan);
									nopHoSoOfflineUrl.setParameter("idCongDanNop",idCongDanNop);
									nopHoSoOfflineUrl.setParameter("idQuyTrinh", idQuyTrinh);
						...
						<tr>
							<td align="left" colspan="2">
								Số CMND <font color="red">(*)</font>:
								<input id="_searchKeyWord" name="_searchKeyWord" value="<%=_searchKeyWord %>" type="hidden" maxlength="12" size="40"/>
								<input id="searchKeyWordOld" name="searchKeyWordOld" value="<%=searchKeyWordOld %>" type="hidden" maxlength="12" size="40"/>
								...
								<input type="hidden" name = "idCongDan" id = "idCongDan" value = "<%=idCongDan%>"/>
								<input type="hidden" name = "idCongDanNop" id = "idCongDanNop" value = "<%=idCongDanNop%>"/>
						...
						<tr>
							<td colspan="2" align="left"><a href="<%=addCongDanRequestUrl.toString() %>"><font size="4">Thêm mới thông tin công dân</font></a></th>
						</tr>
					</table>
				</td>
			</tr>
	...
	<%
	List<CongDan>  result = new ArrayList<CongDan>(); 
	
	if (request.getAttribute("SEARCH_REQUEST_RESULT") != null) {
		 result = (List<CongDan>)request.getAttribute("SEARCH_REQUEST_RESULT");
	} else {
		if (searchKeyWord.trim().length() > 0) {
			FormOfflineBusiness formOfflineBusiness = new FormOfflineBusiness();
			result = formOfflineBusiness.findByCMNDLike(searchKeyWord);
		}
	}
	
	PortletURL iteratorUrl = renderResponse.createRenderURL();
	iteratorUrl.setParameter("statusHS", statusHS);
	iteratorUrl.setParameter("jspPage", "/html/portlet/ipmslistportlet/company/timCongDan.jsp");
	...
	iteratorUrl.setParameter("searchKeyWordOld", searchKeyWordOld);
	iteratorUrl.setParameter("_searchKeyWord", _searchKeyWord);
	iteratorUrl.setParameter("idQuyTrinh", idQuyTrinh);
	iteratorUrl.setParameter("idCongDan",idCongDan);
	iteratorUrl.setParameter("idCongDanNop",idCongDanNop);
	...
	//Paging
	List<CongDan> thuTucHanhChinhs = ListUtil.subList(result, searchContainer.getStart(), searchContainer.getEnd());
	...
	<table cellpadding="0" cellspacing="0"  class="egov-table" width="100%">
			...
			<%
			int i = 0;
			for (CongDan congDan : thuTucHanhChinhs) {
				%>
			<tr>
				<td align="center" style="text-align: center;"><%=(searchContainer.getStart()+i+1)%></td>
				<td align="center" style="text-align: center;"><%=congDan.getSoCmnd() %></td>
				<td align="left"><%=congDan.getTenDayDu() %></td>
				<td align="center" style="text-align: center;"><%=ConvertUtil.parseDateToTring(congDan.getNgaySinh()) %></td>
				<td align="left"><%=congDan.getDiaChiThuongTru() %>,<%=ConvertUtil.getTenDonViHanhChinh(congDan.getDiaChiThuongTruXaId()) %>,<%=ConvertUtil.getTenDonViHanhChinh(congDan.getDiaChiThuongTruHuyenId()) %>,<%=ConvertUtil.getTenDonViHanhChinh(congDan.getDiaChiThuongTruTinhId()) %></td>
				...
				editRequestUrl.setParameter("idCongDan", idCongDan);
				editRequestUrl.setParameter("idCongDanNop",String.valueOf(congDan.getId()));
				...
				editRequestUrl.setParameter("searchKeyWord", searchKeyWordOld);
				editRequestUrl.setParameter("statusHS", statusHS);
				editRequestUrl.setParameter("_searchKeyWord", _searchKeyWord);
				%>
				<a href="<%=editRequestUrl.toString() %>">Công dân nộp đơn</a>
				</td>
			</tr>
				<%
				i++;
