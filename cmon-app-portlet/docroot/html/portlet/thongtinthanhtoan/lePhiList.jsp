...
%>
<portlet:renderURL var ="xacNhanLePhiChuyenKhoan">
	<portlet:param name="typePhi" value="2"></portlet:param>
...
<portlet:renderURL var ="xacNhanLePhiTienMat">
	<portlet:param name="typePhi" value="2"></portlet:param>
	<portlet:param name="parentUrl" value="<%= requestUrl %>"></portlet:param>
	<%-- <portlet:param name="hoSoTTHCCongId" value="1102309"></portlet:param> --%>
	<%-- <portlet:param name="thongTinThanhToanId" value="223"></portlet:param> --%>
...
<portlet:renderURL var ="xacNhanLePhiTrucTuyen">
	<portlet:param name="typePhi" value="2"></portlet:param>
	<portlet:param name="parentUrl" value="<%= requestUrl %>"></portlet:param>
	<%-- <portlet:param name="hoSoTTHCCongId" value="1102309"></portlet:param> --%>
	<%-- <portlet:param name="thongTinThanhToanId" value="223"></portlet:param> --%>
...
<form name="searchForm" method="post" action="<%=portletUrl.toString()%>" >
			...
			<table cellpadding="0" cellspacing="0" class="egov-table" width="100%">
				...
				<%
					...
					List<ThongTinThanhToan> results = ThongTinThanhToanLocalServiceUtil.searchByCoQuanQuanLyHoSo_QT(coQuanQuanLyId, searchProfileCodeKeyword, Constants.ThongTinThanhToan.LE_PHI, start, end);
					long longTotal = ThongTinThanhToanLocalServiceUtil.countByCoQuanQuanLyHoSo_QT(coQuanQuanLyId, searchProfileCodeKeyword, Constants.ThongTinThanhToan.LE_PHI);
				...
				<tr class="bgcolor-<%= bgColor%>">
					...
					<td width="10%">
						<% if (instance.getLoaiThanhToan() == 1 && instance.getXacNhan() == 1) { %>
							<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiTienMat %>','<%= hoSoTTHCCong.getId() %>','<%= instance.getId() %>', true);"><img alt="" src="<%=request.getContextPath()%>/images/xac_nhan_thanh_toan.png" title="Chi tiết thông tin nộp phí tiền mặt"></a>
						<% } %>
					</td>
					<%
					...
					<td width="4%">
						<% if (instance.getLoaiThanhToan() == 2 && instance.getXacNhan() == 1) { %>
							<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiChuyenKhoan %>','<%= hoSoTTHCCong.getId() %>','<%= instance.getId() %>', true);"><img alt="" src="<%=request.getContextPath()%>/images/xac_nhan_thanh_toan.png" title="Chi tiết thông tin nộp phí chuyển khoản"></a>
						<% } %>
					</td>
					<td width="6%">
					...
					<td width="4%">
						<% if (instance.getLoaiThanhToan() == 3 && instance.getXacNhan() == 1) { %>
							<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiTrucTuyen %>','<%= hoSoTTHCCong.getId() %>','<%= instance.getId() %>', true);"><img alt="" src="<%=request.getContextPath()%>/images/xac_nhan_thanh_toan.png" title="Chi tiết thông tin nộp phí trực tuyến"></a>
						<% } %>
					</td>
					<td width="5%">
						<% if (instance.getXacNhan() == 0) { %>
							<%
								...
								case 0:
							%>
									<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiTienMat %>','<%= hoSoTTHCCong.getId() %>','<% if (instance.getNgayTao() != null) {%><%= instance.getId() %><%} else {%>0<%} %>', false);" style="padding:1px;"><span class="icon-20" title="Nhập thông tin nộp phí trực tiếp"></span></a>
<%-- 									<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiChuyenKhoan %>','<%= hoSoTTHCCong.getId() %>','<%= instance.getId() %>', false);" ><img alt="" src="<%=request.getContextPath()%>/images/chuyen_khoan.png" title="Nhập thông tin chuyển khoản"></a>
									<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiTrucTuyen %>','<%= hoSoTTHCCong.getId() %>','<%= instance.getId() %>', false);"><img alt="" src="<%=request.getContextPath()%>/images/thanh_toan_truc_tuyen.png" title="Nhập thông tin thanh toán trực tuyến"></a> --%>
							<%
										break;
								case 1:
							%>
							...
							<%
								...
								case 2:
							%>
								<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiChuyenKhoan %>','<%= hoSoTTHCCong.getId() %>','<% if (instance.getNgayTao() != null) {%><%= instance.getId() %><%} else {%>0<%} %>', false);" style="padding:1px;"><span class="icon-22"title="Nhập thông tin chuyển khoản" ></span></a>
							<%
										break;
									default:
								%>
										<a href="#" onclick="gotoXacNhanPhi('<%= xacNhanLePhiTrucTuyen %>','<%= hoSoTTHCCong.getId() %>','<% if (instance.getNgayTao() != null) {%><%= instance.getId() %><%} else {%>0<%} %>', false);" style="padding:1px;"><span class="icon-21" title="Nhập thông tin nộp phí trực tuyến"></span></a>
								<%
								}
								%>
						<% } %>
